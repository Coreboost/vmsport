  ...
  protected static JJTHblState jjtree = new JJTHblState();private static Integer error_count = 0;
    ...
    public static void main( String[] args ) {
      ...
      if (error_count == 0) {
        ...
        if (exit_code == 0) {
          try {
            IncludeManager.getInstance().setRoot(input_filename, inputStream);
            inputStream = IncludeManager.getInstance().getRootStream();
            parse(true);
            if (exit_code == 0) {
              inputStream = IncludeManager.getInstance().getExpandedStream();
              parse(false);
            }
          } catch (Exception e) {
    ...
    - Write some tests for the include stuff
    ...
    */
    ...
    public static void parse(InputStream iStream, PrintStream oStream, PrintStream eStream) throws IOException, FileNotFoundException {
      outputStream = oStream;
      System.setErr(eStream);
      IncludeManager.getInstance().setRoot(null, iStream);
      inputStream = IncludeManager.getInstance().getRootStream();
      parse(true);
      if (exit_code == 0) {
        inputStream = IncludeManager.getInstance().getExpandedStream();
